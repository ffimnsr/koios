...
- [x] `/activation mention|always` toggle in chat
...
- [x] Session activation modes for group and chat surfaces
...
- [x] Stream queue/throttle modes per channel
...
- [x] `git.*` built-in tools for structured repository operations
...
- [x] `message.send` built-in tool for cross-channel outbound messages
...
- [x] Rich exec approvals UX and lifecycle
...
- [x] Browser automation (Chromium/CDP: snapshots, clicks, form fill, uploads, profiles)
	- Implementation Notes: Koios exposes first-class `browser.*` built-ins over hidden browser backends. Managed and existing-session profiles both route through the active session browser profile, and Koios now adds stable action-oriented wrappers on top of the raw Chrome MCP tool surface instead of exposing raw MCP names directly.
	- Notes: Koios now supports named browser profiles backed by hidden `chrome-devtools-mcp` servers. The active session profile exposes Chrome DevTools MCP actions through `browser.*` aliases while keeping raw MCP tool names out of the catalog.
...
- [x] Browser profile management per session
	- Notes: Sessions can now persist a `browser_profile` override via `session.patch` or `browser.profile.use`, and `browser.profile.list` / `browser.status` surface the effective browser context.
...
- [x] Managed Chromium browser profile for agent use
	- Notes: Managed profiles derive a dedicated Chrome user-data-dir under the Koios workspace and launch `chrome-devtools-mcp` in managed mode by default; attach and remote websocket/browser-url modes are also supported per profile.
...
- [x] Browser status, start, stop, tabs, open, focus, and close commands
	- Notes: Koios now exposes `browser.start`, `browser.stop`, `browser.tabs`, `browser.open`, `browser.focus`, and `browser.close` as first-class built-ins layered over the active Chrome DevTools MCP profile, alongside enriched runtime state in `browser.status`.
...
- [x] Browser actions for click, type, drag, select, hover, scroll, and fill
...
- [x] AI snapshots with stable refs for actions
	- Notes: Koios now exposes `browser.snapshot`, which rewrites Chrome MCP snapshot uids into stable `@eN` refs scoped to the current session/profile/page and lets ref-capable browser actions such as `browser.click`, `browser.type`, `browser.hover`, `browser.fill`, `browser.select`, and `browser.drag` resolve those refs back to the current Chrome uid.
...
- [x] Accessibility and role snapshots
	- Notes: `browser.snapshot` now supports `format:"role"` for compact stable-ref role trees and `format:"accessibility"` for structured accessibility trees plus stable refs derived from the active Chrome MCP snapshot.
...
- [x] Screenshot capture
	- Notes: Koios now exposes `browser.screenshot` for element or full-page captures, accepts stable snapshot refs, and returns the captured image content plus MIME type from the active browser backend.
...
- [x] PDF export
	- Notes: Koios now exposes `browser.pdf`, which exports the active page to PDF when the current browser backend exposes a PDF-capable hidden tool and returns the resulting PDF bytes with MIME metadata.
...
- [x] Multi-profile browser support
	- Notes: Koios now exposes multiple named browser profiles in config and derives one hidden Chrome MCP server per enabled profile. Sessions can inspect available profiles with `browser.profile.list`, switch the active profile with `browser.profile.use`, persist the override via `session.patch`, and see the effective profile in `browser.status`.
...
- [x] Existing-session attach to the user's browser
	- Notes: Koios now treats `existing_session` as a first-class browser profile mode for attaching to a user's already-running local Chrome profile. Doctor checks and browser status output surface attach-specific details such as `user_data_dir`, and the hidden Chrome MCP launcher uses auto-connect mode instead of requiring a managed browser instance.
...
- [x] Cookie and storage inspection and editing
...
- [x] Offline, header, geo, timezone, and device emulation knobs
...
- [x] Browser-targeted SSRF protections
...
- [x] Screen recording / screen snapshot tool
	- Notes: Koios now exposes generic `screen.snapshot` and `screen.record` tools on top of the active browser-backed visual surface. `screen.snapshot` wraps the existing browser screenshot path, while `screen.record` captures a short animated GIF by sampling repeated browser screenshots from the active page or targeted element.
...
- [x] Canvas / A2UI tools (push, reset, eval, snapshot)
	- Notes: Koios now exposes browser-backed `canvas.push`, `canvas.reset`, `canvas.eval`, and `canvas.snapshot` tools. They manage a session-scoped in-page canvas overlay, persist structured canvas state per page, and can return both structured state and an optional screenshot snapshot of the current visual workspace.
...
- [x] Plugin-provided CLI commands
...
- [x] Plugin-provided channels
	...
	- Implementation notes: Koios now supports manifest-backed extension channels through the `channels` capability. An extension can register a webhook-style channel route or a polling loop, translate inbound events into normalized channel messages through MCP tools, participate in the shared channel lifecycle with optional start/shutdown hooks, suppress automatic agent replies per envelope when needed, and reuse the existing `outbound_messages` capability for replies on the same channel id.
...
- [x] Sandbox tool allow/deny list per session type
...
- [x] Elevated bash toggle per session (`/elevated on|off`)
...
- [x] Sandbox-specific least-privilege tool defaults
...
- [x] Gateway lock file (prevent duplicate daemon instances)
...
- [x] Sensitive data filtering in logs (credential/token scrubbing)
...
- [x] Conversation binding approvals
...
- [x] Plugin safety gates for path ownership and world-writable checks
...
- [x] Richer provider fallback and model-switch handoff behavior
		- Status: Implemented via capability-aware fallback ordering plus persisted route handoff state. Koios now prefers fallback entries that better match the request's streaming/tool requirements and favors same-provider fallbacks when practical. Session-scoped provider-owned replay state is now keyed to the resolved provider/model endpoint, so switching provider profiles, base URLs, or models clears incompatible OpenAI replay state before the next turn and re-seeds it on the new route instead of leaking stale provider-owned conversation handles across handoffs.
...
- [x] Lightweight session status tool with per-session model override
		- Status: Implemented via `session.status` and matching websocket RPC support. Koios now exposes a lightweight per-session status payload with the persisted `model_override`, selected provider profile, resolved provider/model/base URL, selection reason, session-scoped route ownership metadata, provider-owned replay state availability, and core session toggles. The same status surface is available through chat tools, websocket RPC, and server capabilities discovery so operators can inspect model handoff state and per-session overrides without reading raw policy storage.
...
- [x] Telegram channel (bot token, long polling/webhooks)
...
- [x] Multi-channel inbox routing (route channels to isolated agents/sessions)
...
- [x] Group/channel mention gating (`mention` vs `always` activation)
...
- [x] Group activation rules (owner-only commands, reply tags)
...
- [x] Per-channel sender allowlist with DM pairing codes
...
- [x] DM pairing flow (unknown sender gets a pairing code; owner approves via CLI)
...
- [x] `pairing approve <channel> <code>` CLI command
...
- [x] Per-channel `dmPolicy` enforcement (pairing / open / closed)
...
- [x] Backward-compatible multi-key LLM profile configuration
	...
	- Implementation plan:
		- [x] Define the public config shape.
			- [x] Add `api_keys = ["..."]` support to the root `[llm]` config while keeping existing `api_key = "..."` valid.
			- [x] Add `api_keys = ["..."]` support to each `[[llm.profiles]]` entry while keeping profile-level `api_key = "..."` valid.
			- [x] Enforce `api_key` and `api_keys` as mutually exclusive fields; return a clear config error when both are defined on the same root or profile entry.
			- [x] Preserve empty-key support only for providers that intentionally do not require credentials, such as local Ollama-compatible deployments.
		- [x] Update runtime config structs and normalization.
			- [x] Extend `internal/config/config.go` with `APIKeys []string` on the root LLM config path.
			- [x] Extend `ModelProfile` with `APIKeys []string`.
			- [x] Add a normalization helper that returns a stable, trimmed, deduplicated key ring from either `api_key` or `api_keys`, never both.
			- [x] Update config validation so authenticated providers fail clearly when the normalized key ring is empty.
		- [x] Update config templates and docs.
			- [x] Update `internal/config/templates/default_config.toml` with commented examples for legacy `api_key` and new `api_keys` usage.
			- [x] Add examples showing either one legacy single key or a multi-key load-balanced list, without mixing both fields in one entry.
			- [x] Document migration behavior so existing single-key configs continue working unchanged.
		- [x] Update hidden-secret support for multiple keys.
			- [x] Update `internal/config/secret.go` to decode hidden secrets in every `api_keys` entry.
			- [x] Keep `llm.profiles.<name>.api_key` as the legacy hidden-secret path.
			- [x] Add stable indexed paths such as `llm.profiles.<name>.api_keys.<index>` for multi-key entries.
			- [x] Add tests that mix plaintext keys, hidden single keys, and hidden multi-key entries.
		- [x] Add provider key-selection infrastructure.
			- [x] Introduce a concurrency-safe credential selector in `internal/provider` that owns key-ring state per provider/profile.
			- [x] Track active peer/session-to-key assignments and current usage counts per key.
			- [x] Assign new peers/sessions to the least-used healthy key.
			- [x] Keep peer/session assignments sticky for their lifetime.
			- [x] Decrement usage when peers disconnect or sessions expire.
			- [x] Fall back to deterministic hashing only when no peer/session identity is available.
			- [x] Define how unhealthy keys are marked and restored after provider auth/rate-limit failures.
		- [x] Wire request-time key selection into providers.
			- [x] Change provider constructors in `internal/provider/provider.go` to pass key selectors instead of a single `apiKey` string.
			- [x] Update OpenAI-compatible request paths in `internal/provider/openai.go` to select the key per request before setting authorization headers.
			- [x] Update Anthropic request paths in `internal/provider/anthropic.go` to select the key per request before setting authorization headers.
			- [x] Preserve existing behavior for single-key profiles by producing a one-key selector.
		- [x] Extend peer/private key storage.
			- [x] Extend `internal/peerllm/store.go` from one encrypted `api_key_enc` value to multiple encrypted key records or an encrypted key list.
			- [x] Preserve the existing `api_key_enc` column during migration for compatibility and rollback-safe reads.
			- [x] Add migration logic that copies an existing single stored key into the new multi-key representation.
			- [x] Make reads prefer the new multi-key representation when present and fall back to the legacy column otherwise.
			- [x] Expose safe result metadata such as `api_key_count`, `has_api_key`, and a masked default key without returning plaintext.
		- [x] Update peer/provider management tools and payloads.
			- [x] Add API/tool input support for setting multiple keys at create/update time.
			- [x] Add safe operations for adding, removing, replacing, and rotating individual keys.
			- [x] Ensure list/get responses only return masked key metadata and never plaintext keys.
			- [x] Preserve existing single `api_key` request payloads for backward compatibility.
		- [x] Add focused tests.
			- [x] Cover config parsing and normalization for single-key, multi-key, duplicate-key, and invalid mixed single+multi-key configs.
			- [x] Cover hidden-secret decoding for root and profile `api_key` / `api_keys` fields.
			- [x] Cover active key usage accounting, least-used assignment, sticky assignment cleanup, and deterministic fallback behavior.
			- [x] Cover provider auth headers proving different peers can use different keys under the same profile.
			- [x] Cover peer LLM store migration from a single encrypted key to multiple encrypted keys.
			- [x] Run `gofmt`, `gopls check` on modified Go files, and `go test` on affected packages before marking complete.
...
## MCP Client Modernization Plan

- [x] Implement a complete modern MCP 2026-07-28 client while keeping only non-deprecated compatibility paths
	- Status note: Koios currently has an MCP client and manager, but it is tool-only and legacy-oriented. `internal/mcp/client.go` exposes only `Initialize`, `ListTools`, `CallTool`, and `Close`; `stdio.go` and `http.go` issue legacy `initialize`, `tools/list`, and `tools/call`; `manager.go` caches only tool definitions; `config.go` and `validate.go` still accept the deprecated HTTP+SSE `sse` transport. This prevents Koios from reading MCP resources such as Monaco `mach1://strategy-spec/schema.json` via `resources/read` even when the server implements resources correctly.
	- Research notes: MCP 2026-07-28 moves modern clients to per-request `_meta` instead of an initialize session, adds `server/discover` as the modern capability/version probe, defines Streamable HTTP as one POST per request with required metadata headers, and keeps resources, prompts, tools, subscriptions, MRTR, pagination, and cache hints as active client responsibilities. Deprecated features must not be adopted: HTTP+SSE transport, roots, sampling, logging, dynamic client registration, and sampling `includeContext: thisServer/allServers`.
	- Suggested Koios shape: build a dual-era MCP client that prefers modern 2026-07-28 and can fall back to legacy `initialize` only for existing stdio/Streamable HTTP servers when the modern probe clearly identifies a legacy server. Do not implement deprecated HTTP+SSE, roots, sampling, logging, or dynamic client registration.
	- Concrete plan:
		1. Add protocol model types in `internal/mcp`: `Implementation`, request `_meta`, `ClientCapabilities`, `DiscoverResult`, expanded `Tool`, `Resource`, `ResourceTemplate`, `ResourceContent`, `Prompt`, `PromptMessage`, content blocks, `InputRequiredResult`, elicitation requests/responses, subscription filters, pagination and caching fields (`resultType`, `ttlMs`, `cacheScope`, `nextCursor`).
		2. Replace the narrow `Client` interface with modern methods: `Discover`, `ListTools`, `CallTool`, `ListResources`, `ListResourceTemplates`, `ReadResource`, `ListPrompts`, `GetPrompt`, `Listen`, `Cancel`, and `Close`. Keep legacy `Initialize` as internal fallback plumbing only.
		3. Create one JSON-RPC core that injects `_meta.io.modelcontextprotocol/protocolVersion`, `_meta.io.modelcontextprotocol/clientInfo`, and `_meta.io.modelcontextprotocol/clientCapabilities` into every modern request, allocates non-null request IDs, parses JSON-RPC errors, treats absent `resultType` as `complete` for older servers, and routes notifications instead of dropping no-ID messages.
		4. Modernize stdio transport: launch subprocess, probe with `server/discover`, select a mutually supported protocol version, fall back to legacy `initialize` only on non-modern errors or timeout, send `notifications/cancelled` for cancellation, route progress/list/resource subscription notifications, close stdin for graceful shutdown, and force-kill only after a bounded timeout.
		5. Replace HTTP implementation with Streamable HTTP only: POST every request to the configured MCP endpoint, set `Accept: application/json, text/event-stream`, set `MCP-Protocol-Version`, `Mcp-Method`, and `Mcp-Name` where required, support both JSON and request-scoped SSE responses, close the response stream for HTTP cancellation, and implement safe Base64 sentinel header encoding.
		6. Implement `x-mcp-header` support for Streamable HTTP tool calls: validate annotations in `inputSchema`, reject malformed HTTP-header annotations from tool catalogs without dropping the whole server, and mirror primitive argument values to `Mcp-Param-*` headers when calling tools.
		7. Delete the deprecated HTTP+SSE client path and stop accepting `transport = "sse"` for new config. Update config comments, validation errors, registry validation, docs, and tests to say `transport` is only `stdio` or `http` (Streamable HTTP endpoint).
		8. Extend `Manager` beyond tools: store per-server discovery metadata, tools, resources, resource templates, prompts, capability flags, cache entries, and active subscriptions. List only capabilities the server advertises, follow pagination until `nextCursor` is absent, and treat cursors as opaque including empty-string cursors.
		9. Add cache support for `server/discover`, `tools/list`, `resources/list`, `resources/templates/list`, `resources/read`, and `prompts/list`. Key cache entries by method plus parameters, respect `ttlMs`, scope `private` cache entries to the authorization context, and invalidate on `notifications/tools/list_changed`, `notifications/resources/list_changed`, `notifications/resources/updated`, and `notifications/prompts/list_changed`.
		10. Add prompt and resource manager APIs plus agent-facing tools: `mcp.search`, `mcp.tool.details`, `mcp.tool.call`, `mcp.resource.list`, `mcp.resource.read`, `mcp.prompt.list`, and `mcp.prompt.get`. Keep existing `mcp__server__tool` direct calls initially, but add progressive discovery mode so large MCP catalogs enter context through search/inspect/execute rather than injecting every schema every turn.
		11. Preserve tool-result semantics: protocol errors remain Go errors, but tool execution errors with `isError: true` are returned to the model as recoverable tool results. Preserve `structuredContent`, `outputSchema`, resource links, embedded resources, image/audio blocks, annotations, and text content.
		12. Implement MRTR for active features: when a request returns `resultType: "input_required"`, surface `elicitation/create` requests to the user, support form and URL modes, validate form responses against the restricted schema, require explicit consent before URL navigation, never prefetch URL-mode links, then retry the original method with `inputResponses` and `requestState`.
		13. Add resource flow coverage for Monaco docs: Koios must be able to list/read `mach1://strategy-spec/*` resources through `resources/read`, not by inventing tool names or using workspace file readers.
		14. Update observability and server status surfaces to include protocol era, selected protocol version, server capabilities, resource count, prompt count, tool count, cache freshness, subscription state, and last connection/protocol error without exposing secrets.
		15. Document migration guidance for configured `sse` servers: replace deprecated HTTP+SSE URL with the server's Streamable HTTP MCP endpoint and set `transport = "http"`.
	- Acceptance criteria:
		- Koios can connect to a modern MCP 2026-07-28 stdio server and Streamable HTTP server using `server/discover` and per-request `_meta`.
		- Koios can list and call tools, list and read resources, list resource templates, list and get prompts, and subscribe to list/resource changes when the server advertises those capabilities.
		- Koios can read Monaco MCP resources such as `mach1://strategy-spec/schema.json` through an agent-accessible `mcp.resource.read` path.
		- Koios rejects new `transport = "sse"` configs with a clear deprecation/migration error and contains no HTTP+SSE client implementation.
		- Koios does not emit or rely on roots, sampling, logging, dynamic client registration, `Mcp-Session-Id`, GET SSE endpoint, DELETE session cleanup, or `Last-Event-ID`.
		- Streamable HTTP requests include required metadata headers and support JSON responses plus request-scoped SSE responses.
		- Tool execution errors with `isError: true` remain visible to the model for self-correction instead of being collapsed into transport failures.
		- Cache TTL and list/resource/prompt invalidation behavior is deterministic and covered by tests.
		- Large MCP catalogs can be exposed through progressive discovery without injecting all tool schemas into every model request.
	- Test plan:
		- Unit-test request `_meta` injection, protocol-version selection, `UnsupportedProtocolVersionError` handling, and absent `resultType` compatibility.
		- Unit-test stdio discovery, notification routing, cancellation, graceful shutdown, and unexpected process exit handling.
		- Unit-test Streamable HTTP headers, `Mcp-Name` Base64 sentinel encoding, JSON response parsing, SSE response parsing, cancellation-by-stream-close, and `x-mcp-header` validation/extraction.
		- Unit-test resource listing, resource template listing, resource reading with text/blob content, resource-not-found handling for `-32602` and older `-32002`, prompt listing, and prompt retrieval.
		- Unit-test pagination including empty-string cursors, cache key construction, `ttlMs=0`, private cache scoping, and notification invalidation.
		- Unit-test MRTR elicitation for form accept/decline/cancel, URL consent behavior, request retry with `inputResponses` and `requestState`, and rejection of unsupported or sensitive form-mode requests.
		- Add integration tests with fake modern stdio and Streamable HTTP MCP servers covering tools, resources, prompts, subscriptions, and request-scoped SSE progress.
		- Add regression test proving `mcp.resource.read` can read a Monaco `mach1://strategy-spec/schema.json` resource through Koios.
		- Run `go test ./internal/mcp ./internal/handler`, `gopls check` on modified Go files, then `go test ./...` before marking complete.
	- References: MCP `Build an MCP client` https://modelcontextprotocol.io/docs/2026-07-28/develop/build-client ; MCP `Client Best Practices` https://modelcontextprotocol.io/docs/2026-07-28/develop/clients/client-best-practices ; MCP `Deprecated Features` https://modelcontextprotocol.io/specification/2026-07-28/deprecated ; MCP `server/discover` https://modelcontextprotocol.io/specification/2026-07-28/server/discover ; MCP `Streamable HTTP` https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http ; MCP `stdio` https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/stdio ; MCP `Resources` https://modelcontextprotocol.io/specification/2026-07-28/server/resources ; MCP `Prompts` https://modelcontextprotocol.io/specification/2026-07-28/server/prompts ; MCP `Tools` https://modelcontextprotocol.io/specification/2026-07-28/server/tools ; MCP `Caching` https://modelcontextprotocol.io/specification/2026-07-28/server/utilities/caching ; MCP `Subscriptions` https://modelcontextprotocol.io/specification/2026-07-28/basic/patterns/subscriptions ; MCP `Elicitation` https://modelcontextprotocol.io/specification/2026-07-28/client/elicitation.
